...
const buildHistoryCommand = (history) => () => {
    ...
    vscode.window
        ...
        const mapLinesToSelections = item.blocks.length === 1 &&
            editor.selections.length === item.lines.length;
...
function activate(context) {
    vscode.workspace.onDidChangeConfiguration(() => {
        const clipboardSize = vscode.workspace
            .getConfiguration("copy-copy-paste")
            .get("size", 25);
        history.setBufferLimit(clipboardSize);
    });
    const persistHistory = vscode.workspace
        .getConfiguration("copy-copy-paste")
        .get("persistHistory", false);
    const history = new History_1.default(persistHistory ? context.workspaceState : undefined);
    const clipboardSize = vscode.workspace
        .getConfiguration("copy-copy-paste")
        .get("size", 25);
    history.setBufferLimit(clipboardSize);
