			...
			<div class="holder">
				<div class="search-page">
					...
					<div class="examples">
						...
						<div class='result-tab active suggestion-tab'>find documents from mongodb in nodejs</div>
					</div>
		...
		if (chatPanel) { 
			...
			chatPanel.webview.onDidReceiveMessage(async (data) => { 
				if (data.command === "sendMessage") {
					...
					let max_iterations = data.continue ? continueObj.max_iterations : 40
					...
					while (iterations < max_iterations && completed == false) {
						try {
							const response = await (0, node_fetch_1.default)(
								"https://useblackbox.io/chat-request-v3",
								{
									method: "POST",
							...
							const result = await response.json()
							iterations += 1
							if (result['completed'] == true || result['response'][0][0] == obj['stream']) {
								continueObj = {}
								completed = true
								postMessage({ command: "removeTyping", id, raw: result['response'][0][0] })
								postMessage({command:"hideBeside"})
							}
							obj['stream'] = result['response'][0][0]
							sleep(100)
							//detect code or plain text

							answerArr = []
							const splitted = obj['stream'].split("```")
							...
							if (!completed) {
								if (!isPaused) {
									...
									continueObj.id = id
									postMessage({ command: "showStream", answer: answerArr, type: { confidence: 0 }, completed, id })
								} else {
		...
		return `
			...
			<body>
				...
				(function() {
					...
					function receiveStream(answerArr, from, time = null, type, completed, id) {
						...
						if (completed){
							allAnswers.push({'blackbox': allText})
							lastAnswer = allText
							showFeedbackToaster()
						}
					}
					...
					window.addEventListener('message', event => {
						...
						if(data.command === "showAnswer"){
							...
							receiveStream(data.answer, from = "BLACKBOX", time = null, data.type, data.completed, data.id)
